...
### 종목 발굴 필터 확장 (2026-08-09)
- [x] 상승(+) 토글 6종(등락률·주간·월간·연간·외국인·기관) — `> 0` 조건. 최소%(min_*)는 `>= 0`이라 보합이 걸려 별도로 뒀다. 네 개를 한 번에 켜는 '모두 상승' 버튼 포함
- [x] '지속 상승추세' 필터 — 연초대비 수익률만으로는 폭락 후 반등도 +로 잡혀(KODEX 건설 YTD +58%, 7월 -12.8%) 추세의 꾸준함을 따로 잰다. `metrics.trend_metrics`가 R²(로그종가 회귀 설명력)·최대낙폭·월승률·20일선 유지율을 계산해 `stock_catalog`에 저장하고 `scanner.SUSTAINED_UPTREND` 임계값으로 거른다
- [x] 낙폭 하한(-2%)으로 현금성·단기채권 ETF 제외 — 머니마켓·CD금리류는 R² 100%·낙폭 0%지만 연 1~2%짜리다
- [x] 추세 지표는 연초 이후 시세 전체가 필요해 YTD 기준가 캐시(딥페이징 생략) 경로를 걷어냈다. 대신 YTD도 매번 실측된다
