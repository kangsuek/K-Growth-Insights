...
- **예외**: 스캐너(`scanner._metrics_for`)의 골든/데드크로스·RSI 존 진입 신호는 `confirmed_prices()`를 우회해 항상 당일 미확정 종가까지 포함한 실시간 기준으로 판정합니다(장중에도 즉시 반영). 같은 함수가 반환하는 종가·등락률·수익률·추세·수급 등 나머지 지표는 여전히 `confirmed_prices()`를 거친 확정 기준입니다 — 한 응답 안에 신호(실시간)와 나머지(확정)가 서로 다른 기준일로 섞여 있다는 점에 유의하세요.
