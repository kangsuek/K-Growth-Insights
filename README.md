...
"연초부터 꾸준히 올랐는가"는 연초대비 수익률(YTD)만으로 판정할 수 없습니다. 폭락 후
반등도 +로 잡히기 때문입니다 — KODEX 건설은 YTD +58%지만 7/07~7/31 구간이 -12.8%였습니다.
그래서 수집 단계에서 네 지표를 함께 계산해 `stock_catalog`에 저장합니다.

| 컬럼 | 뜻 |
|---|---|
| `trend_r2` | 로그종가를 시간에 회귀한 설명력(%) — 직선처럼 올랐는가 |
| `trend_mdd` | 연초 이후 최대 낙폭(%) — 도중에 무너진 적 있는가 |
| `trend_win_rate` | 월별 수익률 중 양수 비율(%) |
| `trend_above_ma` | 종가가 20일 이동평균 위였던 날 비율(%) |

종목 발굴의 **지속 상승추세** 체크박스가 이 값들로 거릅니다. 임계값은
`scanner.SUSTAINED_UPTREND` 한 곳에 있습니다. 낙폭 하한(-2%)은 머니마켓·CD금리처럼
낙폭이 사실상 0이라 R²가 100%로 나오지만 연 1~2%인 현금성 ETF를 제외하기 위한 것입니다.

추세선이 우하향이거나 거래일이 60일 미만이면 값을 만들지 않고 비웁니다(NULL은 필터에서
자동 제외). 이 지표는 연초 이후 시세 전체가 필요해, 지표수집이 종목마다 연초까지
딥페이징합니다.
...
표시 숫자는 항상 천 단위 구분 기호를 사용합니다(단, 거래량은 `1.2K`/`7.9M`처럼 K/M 단위로 축약 표기).
...
- 사용자에게 보이는 숫자는 천 단위 구분 기호(거래량은 예외 — K/M 단위 축약)
